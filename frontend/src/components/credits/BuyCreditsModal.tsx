...
const CREDIT_PACKS: CreditPack[] = [
  ...
  },
  {
    ...
    priceId: process.env.NEXT_PUBLIC_STRIPE_PRICE_BUILDER || "price_builder",
  },
  {
    id: "pro",
    label: "Pro",
...
export function BuyCreditsModal({
  ...
  const handlePurchase = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch("/api/backend/credits/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ price_id: pack.priceId, pack_id: pack.id }),
      });
                  ...
                  >
                    {pack.popular && (
                      <div className="absolute -top-2 left-1/2 -translate-x-1/2">
                        <span className="badge-cyber px-2 py-0.5 rounded-full text-xs">
                          POPULAR
                        </span>
                      </div>
                    )}
                ...
                <div className="grid grid-cols-3 gap-2 text-xs">
                  {[
                    ...
                    <div key={item.label} className="text-center">
                      <div className="text-cyber-400 font-mono font-bold">
                        {item.cost}
                      </div>
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    ...
                    Purchase{" "}
                    {CREDIT_PACKS.find((p) => p.id === selectedPack)?.label}
                    {" — "}
                    {formatCurrency(
                      CREDIT_PACKS.find((p) => p.id === selectedPack)?.price ?? 0
                    )}
                  </>
                )}
              </motion.button>
